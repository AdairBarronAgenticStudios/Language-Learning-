...
const ColorsGame: React.FC = () => {
  ...
  const handleCardClick = (card: GameCard) => {
    if (matchedPairs.includes(card.colorId) || card.id === selectedCard?.id || gameProgress.lives <= 0) return;
    ...
    // Check if it's a match (different cards but same color ID and different languages)
    if (card.colorId === selectedCard.colorId && card.isSpanish !== selectedCard.isSpanish) {
      // Match found
      const newMatchedPairs = [...matchedPairs, card.colorId];
      ...
      if (newMatchedPairs.length === colors.length) {
        playSound.levelComplete();
        completeLevel('level2', newScore);
    ...
    <Box sx={{ p: 3 }}>
      <Paper elevation={3} sx={{ p: 3, mb: 3 }}>
        ...
        <Typography variant="body1" align="center" sx={{ mb: 3 }}>
          Match the Spanish color words with their English translations. 
          ...
          {gameCards.map((card) => (
            <Grid item xs={6} sm={4} md={3} key={card.id}>
              <Card 
                sx={{ 
                  cursor: 'pointer',
                  opacity: matchedPairs.includes(card.colorId) ? 0.7 : 1,
                  transform: selectedCard?.id === card.id ? 'scale(1.05)' : 'none',
                  transition: 'all 0.2s ease',
                  bgcolor: matchedPairs.includes(card.colorId) 
                    ? 'success.light' 
                    : selectedCard?.id === card.id 
                    ? 'primary.light'
                    : 'background.paper',
                  '&:hover': {
                    transform: matchedPairs.includes(card.colorId) ? 'none' : 'scale(1.02)',
                    boxShadow: 3,
                  },
      ...
      >
        ...
        <DialogContent>
          ...
          <Typography>
            You've mastered the colors! Ready for the next challenge?
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button 
